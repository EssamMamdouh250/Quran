...
class _QuranTabState extends State<QuranTab> {
  ...
  void searchsura(String text){
    ...
    for (int i = 0; i < 114; i++) {
      ...
      if (Constantas.englishQuranSurahs[i]
              .toLowerCase()
              .contains(text.toLowerCase()) ||
          Constantas.arabicAuranSuras[i]
              .contains(text)) {
  ...
  Widget buildsuraslistview() {
    return Expanded(
      child: ListView.separated(
        ...
        itemCount: filterList.length, itemBuilder:(context, index) {
        int suraIndex = filterList[index];
        return InkWell(
          onTap: () async{
             List<String> suraVerses = await loadSura(index);
             Navigator.push(context, MaterialPageRoute(builder: (context)=>SuraDetails(
          ...
          child: Row(
            children: [
              ...
              Column(
                ...
                children: [
                  Text(Constantas.englishQuranSurahs [index],
                  style: TextStyle(
                    fontSize: 16,
                    color: AppColors.white,
                    fontWeight: FontWeight.bold,
                  ),),
                  Text('${Constantas.AyaNumber[index]} Verses',
                  style: TextStyle(
                    fontSize: 12,
                    color: AppColors.white,
                    fontWeight: FontWeight.bold,
              ...
              Spacer(),
              Text(Constantas.arabicAuranSuras [index],
              style: TextStyle(
                fontSize: 16,
  ...
  String content =
  await rootBundle.loadString('assets/folder/Suras/${index + 1}.txt');
